...
- 模板：`EDGE7/templates/EDGE-内容模板.xlsx`（网站可下载）
- 工作表名固定：`说明` / `开场` / `go` / `stop` / `finish`
- 网页用 **SheetJS CE**（开源）在浏览器内解析；**不上传服务器**，刷新即回到默认
- 未选表格时使用网站默认 `messages.json`
- 语音仍按编号读站点 `audio/`；表格「图片路径」写入后进入对应阶段 `images` 列表
